...
AddEventHandler('esx:playerLoaded', function(xPlayer)
  ...
  AddEventHandler('pop_university:donDePalabra', function()
	  tengodondepalabra = true
	  Citizen.Trace("Lo tienes bro")
  end)
...
AddEventHandler('esx_vehicleshop:sendVehicles', function (vehicles)
  ...
  RegisterNetEvent('pop_donator:setJob')
  AddEventHandler('pop_donator:setJob', function(getDonator)
	  if getDonator == 1 or getDonator == "1" then
		  donator = true
	  end
  end)
...
function OpenShopMenu()
	...
	for i=1, #Categories, 1 do
		  if Categories[i].name == "donator" then
	...
	for i=1, #Vehicles, 1 do
		  if IsModelInCdimage(GetHashKey(Vehicles[i].model)) then
			  if Vehicles[i].category == "donator" then
				  if donator == true then
	...
	for i=1, #Categories, 1 do
		  ...
		  if category.name == 'donator' then
			  if donator == true then
				  ...
				  for j=1, #categoryVehicles, 1 do
					  ...
					  if tengodondepalabra == true and putted == false then
						  vehicle.price = vehicle.price * 0.8
					  end
	  ...
	  if tengodondepalabra == true then
		  putted = true
	  end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
					...
					if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							...
							if data3.current.value == 'personnal' then
								...
								end, vehicleData.model,tengodondepalabra)
			...
			----------------------------------------------------
			elseif data2.current.value == 'bank' then
				if Config.EnablePlayerManagement then
					...
					if Config.EnableSocietyOwnedVehicles and playerData.job.grade_name == 'boss' then
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm_buy_type', {
							...
							if data3.current.value == 'personnal' then
								...
								ESX.TriggerServerCallback('esx_vehicleshop:buyVehiclebank', function(hasEnoughMoney)
									if hasEnoughMoney then
										IsInShopMenu = false
	...
	while true do
		...
		for k,v in pairs(Config.Zones) do
			if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
				...
				ESX.ShowFloatingHelpNotification("Pulsa [E]~w~ para acceder a la tienda", vector3(v.Pos.x, v.Pos.y, v.Pos.z + 1.5))
			end
		end
	end
end)
